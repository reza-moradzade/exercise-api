...
import { ExerciseModel, ExerciseResponseSchema, PaginationQuerySchema } from '../models/exercise.model.js';
export class ExerciseController {
    ...
    buildPaginationUrls(origin, pathname, currentPage, totalPages, limit, additionalParams = '') {
    ...
    initRoutes() {
        this.controller.openapi(createRoute({
            ...
            description: "Search exercises using fuzzy matching across all fields (name, muscles, equipment, body parts). Perfect for when users don't know exact terms.",
            operationId: 'searchExercises',
            request: {
                query: PaginationQuerySchema.extend({
                    q: z.string().min(1).openapi({
                        title: 'Search Query',
                        description: 'Search term that will be fuzzy matched against exercise names, muscles, equipment, and body parts',
                        type: 'string',
                        example: 'chest push',
                        default: ''
                    }),
            ...
            const { previousPage, nextPage } = this.buildPaginationUrls(origin, pathname, currentPage, totalPages, limit, `q=${encodeURIComponent(q)}&threshold=${threshold}`);
            return ctx.json({
                success: true,
                metadata: {
                    totalPages,
        ...
        this.controller.openapi(createRoute({
            ...
            const { previousPage, nextPage } = this.buildPaginationUrls(origin, pathname, currentPage, totalPages, limit, `${searchParam}${sortParams}`);
            return ctx.json({
                success: true,
                metadata: {
                    totalPages,
        ...
        this.controller.openapi(createRoute({
            ...
            const { offset = 0, limit = 10, search, muscles, equipment, bodyParts, sortBy = 'name', sortOrder = 'desc' } = ctx.req.valid('query');
            const { origin, pathname } = new URL(ctx.req.url);
            ...
            if (muscles)
                queryParams.append('muscles', muscles);
            if (equipment)
                queryParams.append('equipment', equipment);
            if (bodyParts)
                queryParams.append('bodyParts', bodyParts);
            queryParams.append('sortBy', sortBy);
            queryParams.append('sortOrder', sortOrder);
            const { previousPage, nextPage } = this.buildPaginationUrls(origin, pathname, currentPage, totalPages, limit, queryParams.toString());
            return ctx.json({
                success: true,
                metadata: {
                    totalPages,
        ...
        this.controller.openapi(createRoute({
            ...
            const { origin, pathname } = new URL(ctx.req.url);
            const { totalExercises, currentPage, totalPages, exercises } = await this.exerciseService.getExercisesByBodyPart({
                offset,
                limit,
        ...
        this.controller.openapi(createRoute({
            ...
            const { origin, pathname } = new URL(ctx.req.url);
            const { totalExercises, currentPage, totalPages, exercises } = await this.exerciseService.getExercisesByEquipment({
                offset,
                limit,
        ...
        this.controller.openapi(createRoute({
            ...
            const { previousPage, nextPage } = this.buildPaginationUrls(origin, pathname, currentPage, totalPages, limit, `includeSecondary=${includeSecondary}`);
            return ctx.json({
                success: true,
                metadata: {
                    totalPages,
